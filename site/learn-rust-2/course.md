    ...
    chapters:
      - title: "Part 1: The Problem"
        nodes: [p1_1, p1_1b, p1_2, p1_3, p1_4, p1_14, p1_15, p1_16, p1_17,
                p1_18, p1_19, p1_20, p1_20b]
      - title: "Part 2: Distributed Systems and Coherence"
        nodes: [ds_0, p1_12, p1_13, p1_5b, p1_8, p1_8b, p1_5, p1_6, p1_9, p1_10,
                p1_10b, p1_21, p1_11]
      - title: "Part 3a: What the Compiler Needs"
        nodes: [p2_1, p2_2, p2_3, p2_4]
      - title: "Part 3b: What Defeats the Analysis"
        nodes: [p2_5, p2_6, p2_7, p2_8, p2_8b,
                p2_9, p2_10, p2_11, p2_12]
      - title: "Part 3c: Rust's Patches"
        nodes: [p2_13, p2_14, p2_15,
                p2_16, p2_17, p2_18, p2_19, p2_20, p2_21, p2_22]
      - title: "Part 3d: The Borrowing Rule"
        nodes: [p2_23, p2_24, p2_25, p2_26, p2_27, p2_28]
...
This is an interactive graph. Each section ends with an underlined *next.* that shows where you can go. Hover to see the options, then click to continue. Underlined words inside the text link to related topics. Hover for a preview, click to jump. You can also hover any section title to see its connections.
...
These programs contain memory bugs that Rust rejects and C++ compiles.
...
The value moved to `w`, and `v` is gone. This is [ownership](!p2_14 "Why does assigning a value invalidate the original?") in action.
...
The spawned thread might outlive `main`. The closure captures `v` by reference, but the reference could dangle when `main` returns. The fix is to `move` the value into the closure, transferring [ownership](!p2_14 "Where does the value go, and why can't the old scope see it?") to the thread.
...
`r` refers to `x`, and the block ends. `x` is gone, and `r` points to nothing.
...
## ds_0 | Why Distributed Systems Theory
> Data must be distributed. Distributed data decoheres. Preventing decoherence is what distributed systems theory fundamentally solves.

Physics forces data to be distributed. Cores have caches because RAM is too far away. Threads have local copies because the bus is too slow. Servers have replicas because continents are too wide. Every layer of computing distributes data to survive distance.

Distributed data decoheres. A write to the source makes every copy stale. The copies do not know the source changed, and without a protocol the system silently diverges.

Preventing decoherence is the fundamental problem of distributed systems theory. The theory produced a constraint, many readers or one writer, and proved it optimal. The same constraint appears in hardware cache coherence, database locking, and Rust's borrow checker. Each layer enforces it differently, and none escape it.

<!-- edges
-> p1_12 | Where does distribution start? | Physics creates distance. Distance forces copies.
-> p1_8 | What is the constraint? | Many readers or one writer.
-->
...
Every copy is derived data. A representation computed from a source carries a sync obligation. A change to the source makes the copy stale. Keeping copies consistent with their source is a coherence problem. Every layer solves it with a different strategy.
...
> The coherence problem has three dimensions. Where copies live. When they change. How you reach them.
...
**Derived data** propagates coherence problems. A coordinate to a value is derived data. A cached copy is derived data. The source lives in one SPACE, the derived copy in another, and TIME determines when they fall out of sync.
...
> Derived data carries a sync obligation. The holders do not know when they fall out of sync. This is the coherence problem.

Every copy is derived data, and derived data goes stale. The holders do not know.

A coordinate to a value is derived data. A cached copy is derived data. The source lives in one SPACE, the derived copy in another, and TIME determines when they fall out of sync.
...
> Many readers or one writer. The surprise is not the rule. The surprise is that nothing more permissive exists.
...
Rust's design goal is to solve the coherence problem at compile time with zero runtime cost, no garbage collector, and no tracking overhead. The readers-writers rule is the only coherence policy a compiler can enforce by reading the program without executing it.
...
> The hardware enforces the same rule. Every core has its own cache. Writes invalidate every other copy.

The hardware enforces the rule, many readers or one writer, on every CPU.

Every core in a modern CPU has its own cache, a local copy of data from RAM. Multiple cores can cache the same address. Reads produce no conflict because every copy holds the same value. Writes invalidate every other copy. The moment one core writes, every other cached copy of that address is stale.

The hardware enforces coherence with a protocol. A write forces exclusive access. The protocol invalidates all other copies, and all other cores trying to access that address stall. The protocol serializes all access to that address until the write completes.
...
The protocol operates on 64-byte blocks called cache lines. The same state governs two unrelated variables in the same block. Two cores writing to different variables in the same block forces serialization, even though the variables are independent. The hardware calls this false sharing.
...
Two cores share one address. Core 0 reads it, Core 1 reads it, and both hold a cached copy with the current value. No conflict exists.

Core 0 writes, and its copy now holds the new value. Core 1's copy still holds the old value, and Core 1 does not know anything changed.

The protocol must intervene. The protocol notifies every other core holding a copy and invalidates their copies. Only then does Core 0 proceed.

Core 1 must re-fetch the value to read afterward, either from Core 0's cache or from RAM, depending on whether Core 0 has flushed.

Core 1 acquires exclusive access to write. The protocol invalidates Core 0's copy. Now Core 1 is the sole holder. The write proceeds.
...
Two readers are safe. One writer with no readers is safe. One writer plus any reader is forbidden. Two writers are forbidden.
...
The directory tracks two things per entry. A **token** grants write permission to at most one holder. A **reader set** lists processes with read-only copies. Token and reader set cannot coexist for the same entry.
...
The initials spell MESI. Two consequences follow from these states.

**Why flush before sharing?** Shared copies can be silently discarded. If the main store were still stale and all shared copies were silently discarded, the value would be lost forever. Flushing before sharing ensures the main store can always recover the value.

**Why can shared copies be discarded silently?** All shared copies match the main store. The value can always be re-fetched. No message is needed. The directory's reader set can be imprecise. It may list processes that have already silently discarded their copies.
...
The whole protocol is one invariant. Write permission and the reader set cannot coexist for the same entry. Every message exchange either avoids the conflict or resolves it by revoking one side.
...
**Write, process holds nothing.** The process needs the token. The directory revokes all readers with invalidation messages and waits for acknowledgments. Then it grants the token. The process can now write repeatedly with zero messages.

**Write, process holds a shared copy.** The process needs the token. The directory revokes all other readers. Grants the token. No data transfer is needed because the process already has the value.
...
Every transition preserves the [constraint](!p1_8 "Many readers or one exclusive writer."), and the protocol is complete.
...
The protocol has four states. Each state records what this process can do right now without asking anyone.

**Sole writer.** You have been writing, no one else has a copy. Keep writing. **Sole reader.** You are the only one with a copy but have not written yet. Your first write needs no permission. **One of many readers.** Others are reading too. You can read, but writing means they have to go first. **Nothing.** You have nothing and must request access.

The hardware calls these Modified, Exclusive, Shared, Invalid. The initials spell MESI. Four states derive from one rule, and the rest is bookkeeping.
...
In the base protocol, all writes to the same entry are fully serialized. Only one process can hold the token at any moment. Every other process that wants to write must wait. Writes to different entries proceed in parallel because each entry has independent state, but writes to the same entry are strictly sequential.

The "sharing" in "shared memory" is reading only. Message passing between processes serializes writing.
...
The heap is freed at t2. At t3, the program frees it again. The second free corrupts the allocator's bookkeeping. The consequences are unpredictable. Crashes, silent corruption, and security vulnerabilities all follow from the same corrupted state.
...
Two coordinates reach the same space, two threads write at the same time, and no synchronization exists between them. The result depends on which write completes first, or whether they interleave at the byte level.

A data race needs three ingredients. Shared data (SPACE), parallel access (TIME), and at least one writer (COORDINATES). Remove any one and the race disappears.
...
**Buffer overflow.** Coordinates that exceed the bounds of their space. The buffer has 5 elements. The coordinate points to index 7. The space at that address belongs to something else. Rust prevents this at runtime. Vector and slice indexing checks bounds, and the coordinate cannot exceed the space.
...
**Memory leak.** Space outlives all coordinates to it. The coordinate goes out of scope. The allocation remains. No coordinate can reach it, and the program has lost the ability to free it. Rust prevents most leaks with [ownership](!p2_14 "When the owner goes out of scope, the value is dropped automatically."). The owner drops the value when its scope ends. Leaks are still possible via `mem::forget` or reference cycles in `Rc`, but they require deliberate effort.
...
Each language feature targets a [specific interaction between primitives](!p1_20b "Which S×T×C interaction does each feature address?").

<!-- edges
-> p1_20b | Which features address which dimensions? | Each feature targets a specific S×T×C interaction.
...
## p1_20b | Features as S×T×C Solutions
> Each language feature targets a specific interaction between the three primitives.
...
Two bindings. The binding `x` holds a value. The binding `r` holds an address, the location of `x`. `x` is gone after the block ends. `r` still holds the address and the machine will read whatever bytes sit there.
...
The course uses "coordinate" because languages define "pointer" and "reference" differently. `T*` in C++, `&T` in Rust, `*const T` for raw pointers. The conceptual problem is identical. Something tells you where data lives. "Coordinate" abstracts over the language-specific terms.
...
The coordinate `r` is syntactically independent. It has its own declaration, its own location, its own scope. It is semantically dependent. Its purpose is to refer to `x`, and without `x`, the address in `r` points to nothing meaningful. These lifetimes falling out of sync produces a coherence failure.
...
**Stack.** The compiler manages the stack. It determines at compile time how much space each function needs and lays out a stack frame for each call. The compiler considers those slots available for reuse at the end of each scope. The machine does not zero the bytes and does not reclaim them. It adjusts the stack pointer. A coordinate that still holds the old address can read the bytes, and the machine will comply. The language calls this undefined behavior. The machine calls it a normal read.

**Heap.** The runtime manages the heap. An explicit allocation (`malloc`, `new`, `Box::new`) reserves bytes and returns a coordinate. An explicit deallocation (`free`, `delete`, dropping the owner) marks them reusable. The same gap applies. After deallocation, a coordinate that still holds the address can read stale bytes. The language calls this a [use-after-free](!p1_15 "A heap pointer followed after the heap is freed."). The machine calls it a normal read.

The heap adds a problem the stack does not have. The compiler can see the scope boundaries that tie stack space to a known lifetime. Any code holding a heap coordinate can free the space, and heap coordinates can travel into other variables, function arguments, and data structures. No lexical structure in the source text tells the compiler when heap space will die.
...
Source text has hierarchical structure. Functions contain blocks, blocks contain statements, statements contain expressions. A variable declared inside a block is visible only within that block and any nested blocks. Names respect the scope hierarchy, but coordinates do not.
...
The scope hierarchy sees `v` and `r` in `main`'s scope, and `list` in `first`'s scope. Three names, three declarations, three scope boundaries. The scope hierarchy does not see that `r` holds a coordinate into `v`'s space. The coordinate traveled from `main` into `first` as the argument `list`, reached into the vector's first element, and traveled back as the return value bound to `r`. The connection between `r` and `v` crosses two scope boundaries and routes across a function body the caller cannot see.
...
The scope hierarchy tracks names and boundaries. The analysis needs data flow and reclamation across boundaries. RAM has no scope structure at all. Memory is a sequence of numbered cells, and any address can read or write any cell. The compiler bridges these worlds by building a **control flow graph**. Nodes are statements. Edges connect statements where execution can flow from one to the next. Branches create multiple outgoing edges. Loops create backward edges.
...
The second deeper problem is [data that crosses scope boundaries](!p2_7 "The scope tree tracks names, not data flow.").
...
One more obstacle remains. [Types that contain pointers](!p2_8 "Byte-copying a String duplicates the coordinate without duplicating the space.") hide coordinates inside their stack representation.
...
**Garbage collection.** The problem is coordinates pointing to space that no longer exists. GC inverts the relationship. Space is not freed until no coordinates point to it. The runtime traces from roots to find reachable objects. The tradeoff is runtime overhead in memory for bookkeeping, CPU for tracing, and unpredictable pauses.

**Ownership.** Rust patches each obstacle so the compiler can build the complete graph and track every coordinate. Each value has one owner, transfer invalidates the previous binding, and the runtime cost is zero. But the analysis faces a [fundamental limit](!p2_10 "Not just hard. Mathematically impossible.") that no language design can remove.
...
Arbitrary computation resists static prediction, and [no algorithm can avoid this limitation](!p2_10 "Turing proved it in 1936.").
...
The compiler cannot be perfect and must [choose between two failures](!p2_11 "Sound or complete."). [Escape hatches](!p3_4 "Restructure, encode in types, runtime checks, or unsafe.") exist for valid code the compiler rejects.
...
Rust chooses soundness. The [readers-writers rule](!p1_8b "The compiler has no runtime. The readers-writers rule is the only coherence policy enforceable without one.") is the only coherence policy a compiler can enforce without a runtime. The [approximation](!p2_26 "The borrowing rule.") rejects some valid programs, and that is the acceptable cost.
...
Four obstacles face the analysis. [Flat RAM](!p2_6 "The machine provides no structure for the compiler."), [cross-scope data flow](!p2_7 "The scope tree tracks names, not data flow."), [invisible duplication](!p2_8 "Byte-copying duplicates coordinates."), and [undecidability](!p2_10 "Turing proved it in 1936."). One fundamental constraint governs the response. The compiler chooses soundness over completeness. Rust patches each obstacle with a specific design choice.
...
The course includes a companion crate called `notation` (coming soon) with a macro called `explicit!`. Inside the macro, every operation declares what it does.
...
Function lifetime annotations declare that an output holds a coordinate to an input's space. Struct lifetime annotations declare that the struct holds a coordinate to external space. Both tell the compiler which space a coordinate depends on, and the mechanism is the same.
...
C++ uses `const` for both purposes. `const int x = 5` prevents reassignment. `const int& r = x` prevents writing at the reference target. Rust separates them. `mut` on the binding controls reassignment. `&mut` on the coordinate controls write access. The two are orthogonal.
...
An exclusive coordinate suspends the owner for the duration of the coordinate's liveness. The compiler enforces this as part of the [borrowing rule](!p2_26 "Many shared or one exclusive."). The owner resumes when the coordinate expires. `take` permanently invalidates the source. An exclusive coordinate temporarily suspends it.
...
The `explicit!` notation reveals what standard Rust leaves implicit.
...
The element coordinate is derived data. The address `buffer_ptr + index * sizeof(T)` is computed from the Vec's internal state. The derived value becomes stale if that state changes. A formula `a = f(b)` creates a dependency. `a` becomes stale if `b` changes and no recomputation follows.
...
`sort` takes exclusive access to a Vec and rearranges elements in place. `push` takes exclusive access and can reallocate the buffer. Whether `push` reallocates depends on runtime values. The current length, the current capacity, and the allocator's behavior all determine the outcome. This is [undecidable](!p2_10 "No algorithm can determine this for all programs."). No static analysis can determine for all programs whether a particular call to `push` will reallocate.
...
Each program maps to the same pattern. A coordinate exists while the program attempts an operation that could invalidate it.
...
Specialized compilers do analyze decidable cases. Fortran compilers use polyhedral analysis for loop parallelization. ML frameworks require static tensor shapes. The techniques exist and work for specific domains. Rust chose general-purpose consistency over domain-specific precision. The compiler treats all index expressions and all branch conditions the same way. A program that compiles with a constant will still compile when that constant becomes a variable.
...
Signatures encode coordinate connections at struct granularity. Field-level information does not cross function boundaries. Types describe whole structs and encode connections at that level. The compiler sees two borrows of `d` where a human sees two borrows of disjoint fields.
...
The tightening is the price of compile-time proof. The proof is [sound](!p2_11 "Never accept unsafe code."). The compiler accepts no unsafe program. But the proof is approximate, and the compiler rejects some safe programs. The rejected programs are safe because the programmer knows something the compiler cannot verify.

Every rejection is an over-constraint in one of the three dimensions. Every fix relaxes that dimension. The question is always the same. Which dimension is too tight, and how far do you need to loosen it?
...
**What was tightened.** Each allocation has one owner. When the owner goes out of scope, the space is reclaimed. Assignment moves [ownership](!p2_14 "One owner, scope-tied lifetime."), and the source becomes invalid.
...
`Clone` duplicates the data. Each copy is independent. Ownership is trivial because each binding owns its own space. The cost is CPU time and memory.
...
`Copy` opts small types into automatic duplication. `i32`, `bool`, and `f64` have no managed space. Duplicating their bytes produces two independent values.

Arena indices eliminate coordinates entirely. A single collection owns all nodes. Edges are pairs of `usize` integers, freely copyable, with no lifetime tracking. The borrow checker does not track indices.
...
This is the only pattern that resolves all [three dimensions](!p1_13 "Where data lives. When things happen. How you reach it.") simultaneously. The single collection centralizes SPACE. All nodes live as long as the collection, making TIME uniform. Indices are `Copy` integers, so COORDINATES disappear entirely. The Rust ecosystem converges on this pattern for anything graph-shaped. `petgraph`, `slotmap`, and most game engines all use arena indices.
...
Cycles leak memory because each node in the cycle keeps the next alive. `Weak` breaks cycles by providing a non-owning handle.
...
`*const T` and `*mut T` have no ownership semantics. Creating them is safe, but dereferencing them requires `unsafe`. The programmer asserts that the pointed-to space exists and is valid. No one checks.
...
Borrowing instead of consuming means passing `&` where ownership is not needed. The caller keeps ownership.
...
The [aliasing rule](!p2_26 "Many shared or one exclusive.") guarantees that no code writes during shared access. Sometimes a struct needs to write during shared access for a cache lookup, a lazy initialization, or a counter increment.
...
`Mutex` and `RwLock` enforce the same rule across threads. `Mutex` provides exclusive access, and callers block until the lock is free. `RwLock` allows many readers or one writer, the same [constraint](!p1_8 "Many readers or one exclusive writer.") enforced at runtime with blocking.
...
Programs reach for `unsafe` in four situations. The safe patterns impose unacceptable performance costs. A data structure's invariants are provably correct but inexpressible in the type system. FFI boundaries lie outside Rust's ownership model. Intrusive data structures have nodes that contain their own link pointers.
...
```rust
/// SAFETY:
/// SPACE    — `ptr` was allocated by `alloc::alloc` with `layout`
/// TIME     — the allocation has not been deallocated
/// COORD    — no other `&mut` to this memory exists
unsafe {
    let value = &mut *ptr;
    *value = 42;
}
...
<!-- edges
-> p1_1 | Back to the beginning | The course introduction and how to navigate.
-> p3_4 | Remind me of the mirror | Three dimensions, three columns of fixes.
