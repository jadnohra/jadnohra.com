...
bool BoundingVolumeHierarchy::Node::intersects(const PacketRayShot& rayPacket, PacketRayMask& rayPacketState, Packet4& tmin, float Packet4& tmax) {
...
bool BoundingVolumeHierarchy::Node::shootRayPacket(const Constants& cts, const PacketRayShot& rayPacket, PacketRayHit& t, PacketRayMask& rayPacketState, const Packet4& inTmin, const Packet4& inTmax) {
	...
	if (leaf.isValid()) {
		...
		if (leftChild.isValid() && leftChild->intersects(rayPacket, leftRayPacketState, leftTMin, leftTMax)) {
		...
		if (rightChild.isValid() 
			&& rightChild->intersects(rayPacket, rightRayPacketState, rightTMin, rightTMax)
			&& (!hasLeftHit || _mm_movemask_ps(_mm_cmplt_ps(t.t.el4, rightTMin)))
			) {
			...
			rightChild->shootRayPacket(cts, rayPacket, t, rightRayPacketState, rightTMin, rightTMax);
		}
