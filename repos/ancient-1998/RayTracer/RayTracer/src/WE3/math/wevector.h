...
namespace WE {
	...
	struct CtVector3 {
		...
		CtVector3(const FastUnitCoordSys& coordSys, const float& nx, const float& ny, const float& nz);
	...
	public:
		...
		Vector3(const FastUnitCoordSys& coordSys, const float& nx, const float& ny, const float& nz);
		...
		void compMul(const Vector3 &b) {
			x = x * b.x;
			y = y * b.y;
			z = z * b.z;
		};

		void compMul(const Vector3 &b, Vector3& res) {
			res.x = x * b.x;
			res.y = y * b.y;
			res.z = z * b.z;
		};
		...
		Vector3 operator-(const CtVector3 &a) const {
