...
namespace WE {
	...
	public:
		...
		void toTargetPoint(const double* from, float* to) const {

			float fromf[3] = { (float) from[0], (float) from[1], (float) from[2] };

			return toTargetPoint(fromf, to);
		}

		void toTargetVector(const double* from, float* to) const {

			float fromf[3] = { (float) from[0], (float) from[1], (float) from[2] };

			return toTargetVector(fromf, to);
		}
