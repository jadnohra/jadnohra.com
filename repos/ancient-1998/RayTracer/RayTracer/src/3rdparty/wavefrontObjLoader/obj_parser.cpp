...
int obj_parse_vertex_index(int *vertex_index, int *texture_index, int *normal_index, int maxCount = -1)
{
	char *temp_str;
	char *token;
	int vertex_count = 0;
	...
	while((maxCount < 0 || vertex_count < maxCount) && (token = strtok(NULL, WHITESPACE)) != NULL)
	{
...
float obj_parse_float(float defaultVal)
{
	const char* tok = strtok(NULL, WHITESPACE);
...
{
	int indices[3];
	obj_parse_vertex_index(indices, NULL, NULL, 3);
	...
	camera->fov = obj_parse_float(45.0f);
}
...
int obj_parse_obj_file(obj_growable_scene_data *growable_data, const char *filename)
{
	...
	BuffTChars buffTChars;
	BuffChars buffChars;

	FilePath filePath(charToTChar(filename, buffTChars));
...
/*		
	...
	while( fgets(current_line, OBJ_LINE_SIZE, obj_file_stream) )
	{
		...
		else if( strequal(current_token, "mtllib") ) // mtllib
		{
			...
			BufferString filePath(basePath);
			PathResolver::appendPath(filePath, charToTChar(growable_data->material_filename, buffTChars), true, false);

			obj_parse_mtl_file(tcharToChar(filePath.c_tstr(), buffChars), &growable_data->material_list);
			continue;
		}
