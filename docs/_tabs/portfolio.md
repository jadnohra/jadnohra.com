...
- **2024-2025 - Linerate ZK Hardware Acceleration** at <span class="career">[#1 Irreducible](#career)</span>
  - Hardware acceleration framework for zero-knowledge proof systems — unified API supporting emulated (CPU), verilated (RTL simulation), and FPGA device execution modes for ZK cryptographic operations.
  - **Three-Mode Architecture:** Unified device abstraction across emulated (pure software), verilated ([Verilator](https://en.wikipedia.org/wiki/Verilator) RTL simulation), and FPGA hardware. Single codebase with ~42.5K lines Rust, 244 source files. Hardware-software co-design enabling algorithm development on CPU, cycle-accurate validation on Verilator, deployment on FPGA.
  ...
  - <span class="algo">Tech: Rust, Verilator, FPGA (Xilinx), PCIe, DMA, VFIO. Algos: [NTT](https://en.wikipedia.org/wiki/Discrete_Fourier_transform#Number-theoretic_transform), [FRI](https://en.wikipedia.org/wiki/FRI_protocol), [Tensor operations](https://en.wikipedia.org/wiki/Tensor), [Finite fields](https://en.wikipedia.org/wiki/Finite_field), [Polynomial commitments](https://en.wikipedia.org/wiki/Commitment_scheme).</span>
