...
The first demo included me throwing an apple in front of the truck and the truck stopping for it—very high precision. It became known as the Apple Demo. There was also a famous demo where it started raining heavily and we decided to do another run anyway—the function still worked perfectly, no false positives and no false negatives.
...
Runtime was a gap until it became clear that the runtime built for AV cannot be used as-is for simulation. I picked up a rejected candidate who had very fitting background—he had actually failed the C++ test. C++ was not the most critical skill for this as domain knowledge was needed. Reversed decision and hired F., who became lead of the team, contributed all important designs, and now leads similar tech at Apple.
